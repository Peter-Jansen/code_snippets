kernel ParticleExampleKernel : ImageComputationKernel<ePixelWise>
{
  ...
  float3 transform( float3 p )
  {
    ...
    void process() {
        ...
        sample_vel.w = 1.0;
        // sample_vel = view * sample_vel;
        float4 cunt = float4(sample_vel.x, sample_vel.y, sample_vel.z, 0.0);
        // p_color() = float4(sample_vel.x, sample_vel.y, sample_vel.z, 1.0);
        //p_velocity() += float3(sample_vel.x, sample_vel.y, sample_vel.z);
        cunt = camera_matrix * cunt;
        p_velocity() += float3(cunt.x, cunt.y, cunt.z);

            ...
            if (_holdout_toggle) {
                ...
                if (depth > holdout_depth ) { // behind the holdout
                    if (holdout_alpha >=0.99999) holdout_mask = false;
                        ...
                        z_blend = xlerp(z_blend, 1.0, 1-holdout_alpha);
                    }
            }

            if (holdout_mask){
                float2 p = float2(x, y);    // current pixel pos
                float2 d = p - xy;          // vector to center
				d = float2(d.x *pixel_aspect * disc_aspect_ratio, d.y);
                float r2 = dot(d, d);
                float2 cat;
                float cat2;

                // Calculate a render mask, so that we can ignore calculating pixels that would just return zero anyway.
                bool render_mask;
                if (enable_cateye){
                    cat = p - cateye_xy;
                ...
                if (render_mask) {
                    ...
                    // spherical aberration test
                    float normalizedR2 = r2 / effectiveRadius2;
                    float current_sa = spherical_aberration * coc_sign;
                    current_sa *=  (abs_coc/20.0f); // quick test to modulate effect of SA by COC size.
                    // float current_sa = spherical_aberration;
                    // float saWeight = pow(normalizedR2, exp(current_sa));
                    float saWeight = 1.0f;

                    if (current_sa > 0.0f) {
                        // OVER-CORRECTED: Bright Ring
                        saWeight = pow(normalizedR2, current_sa);
                        saWeight *= (1.0f + current_sa); 
                    } 
                    else if (current_sa < 0.0f) {
                        // UNDER-CORRECTED: Soft Center / Glow
                        saWeight = pow(1.0f - normalizedR2, -current_sa);
                        saWeight *= (1.0f + fabs(current_sa));
                    }
                    ...
                    if (enable_cateye) {
                        ...
                        if (enable_cateye_energy_compensation){
                            //  CAT-EYE ENERGY COMPENSATION 
                            // Distance between the two circle centers normalized by radius
                            float cateyeEnergyMultiplier = 1.0f;
                            float d_centers = length(xy - cateye_xy) / effectiveRadius;
                            if (d_centers >= 2.0f) {
                                render_mask = false; // No overlap at all
                            } else if (d_centers > 0.0f) {
                                // High-performance circular asymmetric lens intersection area approximation
                                ...
                                // Scale energy up inversely proportional to the area lost
                                if (fraction_remaining > 0.01f) {
                                    cateyeEnergyMultiplier = 1.0f / fraction_remaining;
                                }
                            }
                            
                            c = c*1.0f+((cateyeEnergyMultiplier-1.0)*cateye_energy_compensation);
                        }
                    ...
                    // Create the disk with softness 
                    float w = 1.0f - smoothstep(soft_thresh, effectiveRadius2, r2);

                    // Apply all the other effects/modulations
                    w *= c;
                    w *= saWeight;
                    w *= holdout_opacity;

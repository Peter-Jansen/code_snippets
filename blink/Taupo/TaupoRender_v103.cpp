...
inline float3 randomv2(float seed){
  ...
  void init() {
    ...
    pixelsPerWorldUnit = (render_res.x*0.5) / haperture; // RENDERED RESOLUTION WIDTH, not overscanned image width.
    ...
    clip_matrix = p_matrix * m * view;
    clip_matrix_prev = p_matrix_prev * m * view_prev;
    clip_matrix_next = p_matrix_next * m * view_next;
...
void renderPoint( float2 xy, float4 pcolor, float pointSize, float world_size, float coc, float falloff, float depth )
{
    // if(_holdout_toggle){
    // float4 read_holdout = holdout(xy.x,xy.y);
    // if ((depth - world_size) > read_holdout.x) return;
    // }
    ...
    for (int y = minY; y <= maxY; y++) {
        for (int x = minX; x <= maxX; x++) {
            ...
            if (_holdout_toggle) {
                ...
                holdout_mask = !(depth > holdout_depth && holdout_alpha > 0.99999) ;
                holdout_opacity = 1.0f;


                if (depth > holdout_depth ) { // behind the holdout
                    if (holdout_alpha >=0.999999) holdout_mask = false;
                    else{
                        holdout_mask = true;
...
float2 quadraticInterpolation(float2 p0, float2 p1, float2 p2, float t) {
  ...
  void process(int2 pos) {
    ...
    */
    ...
    int steps;
    if (_maxSteps <= 1){
        renderPoint(xy,render_col , size, world_size, coc, depth_fade, ndc.z );
    }
    else{
        // float blur_length = (length(xy - xy_prev) + length(xy - xy_next) * _shutter );
        ...
        for (int step = 0; step <= steps; step++){
            ...
            renderPoint(xy_render,render_col, size_render, world_size, coc_render, depth_fade, ndc.z ); 
        }
    }

kernel Taupo_Render : ImageComputationKernel<ePixelWise> {
...
param:
    ...
    float cateye_energy_compensation;
...
local:
    ...
    bool enable_cateye;
    bool enable_cateye_energy_compensation;
    bool enable_spherical_abberation;
  ...
  void define(){
    ...
    defineParam(max_defocus, "max defocus size", 200.0f);
...
bool isInsideFrustum(float4 position, float padding) {
  ...
  // also named... not lerp... because foundry added their own lerp in nuke 16 which causes a reassignment error.
...
inline float3 randomv2(float seed){
  ...
  void init() {
    ...
    sqSize = src_w / 2;
    img_center = float2(float(overscan_res.x) * 0.5, float(overscan_res.y) * 0.5);
    ...
    enable_cateye_energy_compensation = cateye_energy_compensation != 0.0;
    enable_spherical_abberation = spherical_aberration != 0.0;
	

	// start shutter
	if (shutter_type == 0) { 
		shutter_start = 0.5;
		shutter_end = 0.5 + (_shutter*0.5);
	}
	// center shutter
	else if (shutter_type == 1) { 
		shutter_start = 0.5 - (_shutter*0.25);
		shutter_end = 0.5 + (_shutter*0.25);
	}
	// end shutter
	else if (shutter_type == 2) {
		shutter_start = 0.5 - (_shutter*0.5);
		shutter_end = 0.5;
	}
    ...
    float x_scale = float(overscan_res.x) * 0.5;
...
float CoC(float depth, float f, float fd){
    ...
    float coc = ((fd - scene_depth) * pow(f, 2) / (fStop * scene_depth * (fd - f)));
    float coc_pixels = (coc / haperture * float(render_res.x) );
    return coc_pixels;
}
...
// Optimized version: uniform cat-eye math pulled out of the pixel loops
void renderPoint( float2 xy, float4 pcolor, float pointSize, float coc, float falloff, float depth )
{
    ...
    float coc_sign = (coc >= 0.0f) ? 1.0f : -1.0f;
    float abs_coc = fabs(coc);
    
    // 1. Calculate effective radius and energy scale
    if (enable_dof){
        effectiveRadius = pointSize + abs_coc;
    ...
    // 2. PRE-COMPUTE UNIFORM CAT-EYE VARIABLES (Once per particle)
    float2 cateye_xy = xy; 
    float cateyeEnergyMultiplier = 1.0f;
    bool visible_by_cateye = true;

    if (enable_cateye){
        float2 cat_vec = xy - img_center;
        float cateye_limit = cateye_amount * effectiveRadius * 2.0f;
        
        float2 shift_dir = cat_vec / img_radius;
        float total_aspect = disc_aspect_ratio * pixel_aspect;

        if (total_aspect >= 1.0f) {
            cateye_xy.x = xy.x + (shift_dir.x * cateye_limit / total_aspect);
            cateye_xy.y = xy.y + (shift_dir.y * cateye_limit);
        } else {
            cateye_xy.x = xy.x + (shift_dir.x * cateye_limit);
            cateye_xy.y = xy.y + (shift_dir.y * cateye_limit * total_aspect);
        }

        if (enable_cateye_energy_compensation){
            // Calculate distance between the two circle centers normalized by radius
            float d_centers = length(xy - cateye_xy) / effectiveRadius;
            
            if (d_centers >= 2.0f) {
                visible_by_cateye = false; // No overlap at all between bokeh loops
            } else if (d_centers > 0.0f) {
                // acos and sqrt now run EXACTLY once per particle instead of per pixel!
                float half_d = d_centers * 0.5f;
                float area_intersection = 2.0f * (acos(half_d) - half_d * sqrt(1.0f - half_d * half_d));
                float fraction_remaining = area_intersection / 3.14159265f;
                
                if (fraction_remaining > 0.01f) {
                    cateyeEnergyMultiplier = 1.0f / fraction_remaining;
                }
            }
        }
    }

    // Early out if cat-eye optimization determines zero overlap
    if (!visible_by_cateye) return;

    // 3. Establish pixel bounding box
    int intRadiusX = int(ceil(effectiveRadius / disc_aspect_ratio / pixel_aspect));
    int intRadiusY = int(ceil(effectiveRadius));
    int minX = max(0, int(xy.x) - intRadiusX);
    int maxX = min(_w - 1, int(xy.x) + intRadiusX);
    int minY = max(0, int(xy.y) - intRadiusY);
    int maxY = min(_h - 1, int(xy.y) + intRadiusY); 

    float soft_thresh = effectiveRadius2 * (1.0f - softness);
    float cateye_soft_thresh = effectiveRadius2 * (1.0f - cateye_softness);
        ...
        for (int x = minX; x <= maxX; x++) {
            ...
            if (_holdout_toggle) {
                ...
                if (depth > holdout_depth) { 
                    if (holdout_alpha >= 0.99999f) holdout_mask = false;
                    else {
                        holdout_mask = true;
                        holdout_opacity = 1.0f - holdout_alpha;
                    }
                }
                else {
                    float dist_from_holdout = holdout_depth - depth;
                    z_blend = smoothstep(0.0f, z_blend_distance, dist_from_holdout);
                    z_blend = xlerp(z_blend, 1.0f, 1.0f - holdout_alpha);
                }
            }
            ...
            if (holdout_mask){
                float2 p = float2(x, y);
                float2 d = p - xy;
                d = float2(d.x * pixel_aspect * disc_aspect_ratio, d.y);
                float r2 = dot(d, d);

                bool render_mask = r2 < effectiveRadius2;
                float cat2 = 0.0f;

                if (render_mask && enable_cateye){
                    float2 cat = p - cateye_xy;
                    cat = float2(cat.x * pixel_aspect * disc_aspect_ratio, cat.y);
                    cat2 = dot(cat, cat);
                    render_mask = cat2 < effectiveRadius2;
                }
                ...
                if (render_mask) {
                    ...
                    float saWeight = 1.0f;
                    if (enable_spherical_abberation){
                        float normalizedR2 = r2 / effectiveRadius2;
                        float current_sa = spherical_aberration * coc_sign;
                        current_sa *= (abs_coc / 20.0f); 
                        

                        if (current_sa > 0.0f) {
                            saWeight = pow(normalizedR2, current_sa) * (1.0f + current_sa); 
                        } 
                        else if (current_sa < 0.0f) {
                            saWeight = pow(1.0f - normalizedR2, -current_sa) * (1.0f + fabs(current_sa));
                        }
                    }

                    // Softness and cat-eye masking
                    float c = 1.0f;
                    if (enable_cateye) {
                        c = 1.0f - smoothstep(cateye_soft_thresh, effectiveRadius2, cat2); 
                        if (enable_cateye_energy_compensation){
                            // Use pre-computed multiplier instead of calculating area here
                            c *=  1.0f + ((cateyeEnergyMultiplier - 1.0f) * cateye_energy_compensation);
                            // float comp_factor = 1.0f + ((cateyeEnergyMultiplier - 1.0f) * cateye_energy_compensation);
                            // c *= comp_factor;
                        }
                    }

                    float w = 1.0f - smoothstep(soft_thresh, effectiveRadius2, r2);
...
float2 quadraticInterpolation(float2 p0, float2 p1, float2 p2, float t) {
  ...
  void process(int2 pos) {
    ...
    */
    ...
    // So we set up some batching, so that multiple pixels of the cache image will be sampled per dst pixel.
    ...
    float4 read_pos_active = src(cache_x, cache_y);
	// float4 read_util = util(cache_x, cache_y);
	
    
	int active = read_pos_active.w;
    // Return if the id=0, ie, there's no particle
    if(!active) return; 
    //Read position and ID for the prev and next frames
    float4 read_pos_active_prev = src_prev(cache_x, cache_y);    
    float4 read_pos_active_next = src_next(cache_x, cache_y);   
    int active_prev = read_pos_active_prev.w;
    int active_next = read_pos_active_next.w;
    ...
    // Create P attributes (split from active)
    float3 P = float3(read_pos_active.x, read_pos_active.y, read_pos_active.z);
    float3 P_prev = float3(read_pos_active_prev.x, read_pos_active_prev.y, read_pos_active_prev.z);
    float3 P_next = float3(read_pos_active_next.x, read_pos_active_next.y, read_pos_active_next.z);


    // Handle new and dying particles
    if (!active_prev){                                      // if the particle is new, ie, it didn't exist last frame.
        float3 vel = P_next - P;
        P_prev = P - vel;
    }
    if (!active_next){                                     // if the particle is about to die, ie, won't exist next frame
        float3 vel =  P - P_prev;
        P_next = P+vel;
    }
    if (!active_next && !active_prev){                      // if the particle only exists on 1 frame
        P_prev = P;
        P_next = P;
    }
    ...
    coc = max(-max_defocus,min(max_defocus, CoC(ndc.z, focal, scene_focus_distance)));
    coc_prev = max(-max_defocus,min(max_defocus, CoC(ndc_prev.z, focal_prev, scene_focus_distance_prev)));
    coc_next = max(-max_defocus,min(max_defocus, CoC(ndc_next.z, focal_next, scene_focus_distance_next)));
    }
    ...
    if (_maxSteps <= 1 || _shutter == 0.0f){
        ...
        for (int step = 0; step <= steps; step++){
            ...
            float coc_render = xlerp(coc_prev, coc_next, shutter_step); 
            float size_render = xlerp(size_prev, size_next, shutter_step);

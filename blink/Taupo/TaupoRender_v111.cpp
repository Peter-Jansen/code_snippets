...
// float CoC(float depth, float f, float fd){
//     float scene_depth = depth * scene_multiplier;
//     float coc = ((fd - scene_depth) * (f * f) / (fStop * scene_depth * (fd - f)));
            ...
            if (holdout_mask){
                ...
                if (render_mask) {
                    ...
                    if (current_sa > 0.0f) {
                        ...
                        // As current_sa increases, the center carves out
                        saWeight = pow(normalizedR2, current_sa);

                        // Normalization to keep brightness
                        saWeight *= (1.0f + current_sa); 
                    } 
                    else if (current_sa < 0.0f) {
                        // UNDER-CORRECTED: Soft Center / Glow
                        // We invert the radius so 1.0 is at the center
                    ...
                    // saWeight *= (current_sa + 2.0f) / 2.0f;
                    // saWeight = max(0.0f, saWeight); // Safety clamp

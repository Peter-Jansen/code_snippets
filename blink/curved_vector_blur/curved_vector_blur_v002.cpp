...
local:
  ...
  void process(int2 pos) {
    ...
    int steps = max(1, ceil(blur_length * _quality));
    ...
    // Local accumulation buffer (fixed size to help compiler)
    float4 accum = float4(0,0,0,0);
    const int MAX_STEPS = 64; // adjust to your max _quality
    float4 samples[MAX_STEPS];

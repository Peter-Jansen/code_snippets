...
s = float4(p.x < 0.0, p.y < 0.0, p.z < 0.0, p.w < 0.0);
...
float fbm(float4 p, int octaves, float persistence, float lacunarity) {
float sum = 0.0;
float amplitude = 1.0;
float frequency = 1.0;
...
kernel Grok4D : ImageComputationKernel<ePixelWise> {
...
local:
  ...
  void process() {
    float4 in_pos = src();
  dst() = float4(fbm(in_pos, octaves, persistence, lacunarity));
  // dst() = float4 ( ridged4d(in_pos.x,in_pos.y,in_pos.z, in_pos.w, octaves, lacunarity, gain) );
    }

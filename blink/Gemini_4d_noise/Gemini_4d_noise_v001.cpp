...
// Replace your existing noise4D with this:
  float noise4DALT(float4 p) {
    ...
    // To do true 4D interpolation, we need to sample 16 corners.
    // For a lightweight Blink version, we'll interpolate 3D slices across the 4th dimension.
    
    auto hash = [&](float4 v) {
      float4 p = mod(v, 289.0f);
    ...
    // Manual Quadrilinear Interpolation (LERP)
    // Front slice (w0)
    float n0000 = hash(float4(Pi0.x, Pi0.y, Pi0.z, Pi0.w));
    float n1000 = hash(float4(Pi1.x, Pi0.y, Pi0.z, Pi0.w));
    float n0100 = hash(float4(Pi0.x, Pi1.y, Pi0.z, Pi0.w));
    float n1100 = hash(float4(Pi1.x, Pi1.y, Pi0.z, Pi0.w));
    float n0010 = hash(float4(Pi0.x, Pi0.y, Pi1.z, Pi0.w));
    float n1010 = hash(float4(Pi1.x, Pi0.y, Pi1.z, Pi0.w));
    float n0110 = hash(float4(Pi0.x, Pi1.y, Pi1.z, Pi0.w));
    float n1110 = hash(float4(Pi1.x, Pi1.y, Pi1.z, Pi0.w));

    // Back slice (w1)
    float n0001 = hash(float4(Pi0.x, Pi0.y, Pi0.z, Pi1.w));
    float n1001 = hash(float4(Pi1.x, Pi0.y, Pi0.z, Pi1.w));
    float n0101 = hash(float4(Pi0.x, Pi1.y, Pi0.z, Pi1.w));
    float n1101 = hash(float4(Pi1.x, Pi1.y, Pi0.z, Pi1.w));
    float n0011 = hash(float4(Pi0.x, Pi0.y, Pi1.z, Pi1.w));
    float n1011 = hash(float4(Pi1.x, Pi0.y, Pi1.z, Pi1.w));
    float n0111 = hash(float4(Pi0.x, Pi1.y, Pi1.z, Pi1.w));
    float n1111 = hash(float4(Pi1.x, Pi1.y, Pi1.z, Pi1.w));

    // Lerp everything together (X, then Y, then Z, then W)

...
void copyArray(float from[], float to[]){
    ...
    int clipLine3Points(float4 p0, float4 p1, float4 p2, float arr[]) {
        // Initialize output array
        float result[] = {p0.x, p0.y, p0.z, p0.w,
                          p1.x, p1.y, p1.z, p1.w,
                          p2.x, p2.y, p2.z, p2.w};
        
        // First, perform perspective division if w != 0
        float4 v0 = p0;
        float4 v1 = p1;
        float4 v2 = p2;
        ...
        // Check if all points are outside the same boundary
        // if ((v0.x < -1.0 && v1.x < -1.0 && v2.x < -1.0) ||  // Left
        //     (v0.x > 1.0 && v1.x > 1.0 && v2.x > 1.0) ||    // Right
        //     (v0.y < -1.0 && v1.y < -1.0 && v2.y < -1.0) || // Bottom
        //     (v0.y > 1.0 && v1.y > 1.0 && v2.y > 1.0) ||    // Top
        //     (v0.z < -1.0 && v1.z < -1.0 && v2.z < -1.0) || // Near
        //     (v0.z > 1.0 && v1.z > 1.0 && v2.z > 1.0)) {    // Far
        // }
        
        // Simple Cohen-Sutherland style clipping for two segments
        float t0 = 0.0;
        float t1 = 1.0;
        float t2 = 0.0;
        float t3 = 1.0;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            // First segment (p0-p1)
            float delta1 = v10 - v00;
            if (delta1 != 0.0) {
                float tNear = (-1.0 - v00) / delta1;
                float tFar = (1.0 - v00) / delta1;
                if (delta1 < 0.0) {
                    float temp = tNear;
                    tNear = tFar;
                    tFar = temp;
            ...
            // Second segment (p1-p2)
            float delta2 = v20 - v10;
            if (delta2 != 0.0) {
                float tNear = (-1.0 - v10) / delta2;
                float tFar = (1.0 - v10) / delta2;
                if (delta2 < 0.0) {
                    float temp = tNear;
                    tNear = tFar;
                    tFar = temp;
        ...
        // Check if segments are completely clipped
        // if (t0 > t1 || t2 > t3) {
        //     return 0; 
        // }
        
        // Populate result array with clipped points
        
        float4 cpt0 = p0 + (p1 - p0) * t0;  // First point
        float4 cpt1 = p1;                   // Middle point
        float4 cpt2 = p1 + (p2 - p1) * t3;  // Last point
        float clipped_pts[] = {cpt0.x,cpt0.y,cpt0.z,cpt0.w,
                  cpt1.x,cpt1.y,cpt1.z,cpt1.w,
                  cpt2.x,cpt2.y,cpt2.z,cpt2.w};

        copyArray(clipped_pts, arr);
        // isVisible = true;
  ...
  void process(int2 pos) {
    ...
    */
    ...
    // if (!isInsideFrustum(clip,clip_padding) || !isInsideFrustum(clip_prev,clip_padding) || !isInsideFrustum(clip_next,clip_padding)){
        // float line[12];
        // clipLine3Points(clip_prev, clip, clip_next, line);
        // clip_prev = float4(line[0], line[1], line[2], line[3]);
        // clip =      float4(line[4], line[5], line[6], line[7]);
        // clip_next = float4(line[8], line[9], line[10], line[11]);
    // }

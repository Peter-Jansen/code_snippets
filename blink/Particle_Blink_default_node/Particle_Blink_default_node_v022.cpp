...
//   }  
    ...
    float screenSpaceSize(float f, float p_size, float distance){
        float particleSizeOnImagePlane = (f * p_size) / distance;
        float particleSizeInPixels = (particleSizeOnImagePlane * pixelsPerWorldUnit);
    return particleSizeInPixels;
  }  
  ...
  void process(int2 pos) {
    ...
    */
    ...
    if (dist<0.0f){ return; }
        ...
        float size = max(_minSize, min(100.0f, screenSpaceSize(focal, incoming_size, ndc.z)));
        float size_prev = max(_minSize, screenSpaceSize(focal_prev, incoming_size, ndc_prev.z));
        float size_next = max(_minSize, screenSpaceSize(focal_next, incoming_size, ndc_next.z));
        ...
        if (_maxSteps <= 1){
            ...
            adaptive_from_size = min(32, ceil(fabs(coc_prev-coc_next)*2.0));
            ...
            for (int step = 0; step <= steps; step++){
                ...
                renderPoint(xy_render,render_col , size, coc_render, depth_fade, dist );
            }
        }
        ...
        //dst(10,1) = float4(fabs(size_prev ));
        ...
        // dst(10,10) = max( dst(10,10), float4(steps));
        // dst(20,20) = float4(size_prev);
        // dst(21,21) = float4(size_next);
        // dst(22,22) = float4(adaptive_from_size);

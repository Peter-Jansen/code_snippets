...
//   }  
  ...
  void process(int2 pos) {
    ...
    */
    ...
    // if the batch
    if (batch_start.x > src_w && batch_start.y > src_h) return;
    ...
    int duplicates_pp = int(read_util.z);
    float duplicate_radius_pp = read_util.w;
    ...
    float size_prev = max(_minSize, screenSpaceSize(focal_prev, world_size, ndc_prev.z));
    float size_next = max(_minSize, screenSpaceSize(focal_next, world_size, ndc_next.z));
    ...
    if (_maxSteps <= 1){
        ...
        for (int step = 0; step <= steps; step++){
            ...
            renderPoint(xy_render,render_col , size, coc_render, depth_fade, dist ); // using hero dist, not duplicated particle dist!!
        }
    }

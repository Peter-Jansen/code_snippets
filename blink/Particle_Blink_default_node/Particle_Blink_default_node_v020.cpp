...
void renderPoint( float2 xy, float4 pcolor, float pointSize, float coc, float falloff, float depth )
{
    ...
    // float2 cateye_xy = float2( xy.x + cateye_lens_pos *  cateye_amount, 
    //                            xy.y + cateye_lens_pos *  cateye_amount );
    ...
    float cateye_lens_pos = length(float2(xy - img_center)) / img_radius;
    float cateye_limit = cateye_amount * effectiveRadius;
    float2 cateye_xy = xy + (((xy - img_center) / img_radius) * cateye_limit);
    ...
    for (int y = minY; y <= maxY; y++) {
        for (int x = minX; x <= maxX; x++) {
            ...
            float cat2 = dot(cat, cat);

            // if (r2 < effectiveRadius2) {
            if (r2 < effectiveRadius2 && cat2 < effectiveRadius2) {
                float c = 1.0 - smoothstep( .8, effectiveRadius2, cat2);
...
float2 quadraticInterpolation(float2 p0, float2 p1, float2 p2, float t) {
  ...
  void process(int2 pos) {
    ...
    */
    ...
    if(id == 0) return;                                           // also don't bother if the id=0, ie, there's no particle
    ...
    float4 read_util = util(pos.x, pos.y);

    float nage = read_util.w;
    ...
    if (dist<0.0f){ return; }
        ...
        if (pt !=0){
            // P += randomv2(float(pt) + float(id)*0.02)*_duplicate_radius * (1.0-nage);
            // P_prev += randomv2(float(pt) + float(id)*0.02)*_duplicate_radius * (1.0-nage);
            // P_next += randomv2(float(pt) + float(id)*0.02)*_duplicate_radius * (1.0-nage);
        ...
        float incoming_size = read_util.x * _size;
        float size = max(_minSize, min(100.0f, screenSpaceSize(focal, pixelsPerWorldUnit, incoming_size, ndc.z)));
        float size_prev = min(100.0f, screenSpaceSize(focal, pixelsPerWorldUnit, incoming_size, ndc_prev.z));
        float size_next = min(100.0f, screenSpaceSize(focal, pixelsPerWorldUnit, incoming_size, ndc_next.z));
        ...
        if (_maxSteps == 1){
            renderPoint(xy,render_col , size, coc, depth_fade, dist );
        }
        else{
            float blur_length = (length(xy - xy_prev) + length(xy - xy_next) * _shutter );
            int steps = max(2 , min(_maxSteps, int(ceil(blur_length/max(2.0f, min(size_prev, size_next)+coc)*2.0/_spacing))));
            steps -= 1;
            ...
            for (int step = 0; step <= steps; step++){
                ...
                renderPoint(xy_render,render_col , size, coc, depth_fade, dist );
            }
        }
